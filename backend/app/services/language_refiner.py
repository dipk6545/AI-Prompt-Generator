...
"""
    ...
    async def refine_prompt(self, structured_prompt: str, provider: str, api_key: Optional[str] = None, ollama_model: Optional[str] = None) -> str:
        ...
        if provider_upper == "OLLAMA":
            ...
            # We bypass the complex JSON logic of `optimize_prompt_llm` and just do a direct call.
            # But wait, `optimize_prompt_llm` requires specific prompt_templates and returns JSON.
            # I will just write a tiny direct HTTP client or use the existing logic if it supports raw output.
            # Let's import the specific provider handlers directly.
            ...
            if provider_upper == "GROQ":
                ...
                refined = await call_openrouter(structured_prompt, api_key, system_prompt)
                return refined.strip()
            else:
